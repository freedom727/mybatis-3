...
public class UnpooledDataSourceFactory implements DataSourceFactory {
  ...
  public void setProperties(Properties properties) {
    ...
    // 创建 dataSource 对应的 MetaObject 对象，使用metaDataSource包装了dataSource，方便使用反射去操作，毕竟dataSource这里的类型是接口DataSource，而不是实际的类
    ...
    for (Object key : properties.keySet()) {
      ...
      if (propertyName.startsWith(DRIVER_PROPERTY_PREFIX)) {// 以 "driver." 开头的配置
        ...
        // 这里dataSource是UnpooledDataSource类型的，这里的metaDataSource相当于通过反射去设置datasource的属性
        // 初始化到 MetaObject 中
  ...
  /**
   * property中的属性值都是String，这里就只是简单地根据属性类型将值进行转化罢了
   */
